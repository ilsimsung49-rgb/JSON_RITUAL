...
st.set_page_config(page_title="JSON RITUAL v9.9", page_icon="👹", layout="wide")

# ==========================================================
#  PROJECT JSON RITUAL v9.9 [USER COMMAND: 2 BOXES ONLY]
# ==========================================================
...
def inject_styles():
    ...
    /* ABSOLUTE FORCE VISIBILITY ON CODE BLOCKS */
    div[data-testid="stCodeBlock"] { background-color: #000000 !important; border: 2px solid #FFE800 !important; border-radius: 12px; }
    div[data-testid="stCodeBlock"] pre { background-color: #000000 !important; padding: 30px !important; line-height: 2.2 !important; }
    div[data-testid="stCodeBlock"] code { color: #FFE800 !important; -webkit-text-fill-color: #FFE800 !important; font-family: 'Noto Sans KR', sans-serif !important; font-size: 1.4rem !important; }
    div[data-testid="stCodeBlock"] button { background-color: rgba(255, 232, 0, 0.4) !important; color: #000 !important; opacity: 1 !important; border-radius: 4px; }
    ...
    .stButton > button:hover { background: #FFE800 !important; color: #000 !important; box-shadow: 0 0 30px #FFE800; }
...
def main():
    ...
    st.markdown('<div style="color:#FFE800; text-align:center; letter-spacing:8px; margin-bottom:40px;">[ FINAL MASTER v9.9 ]</div>', unsafe_allow_html=True)
    ...
    with t3:
        if st.button("🔥 INVOKE THE FINAL RITUAL"):
            ...
            st.session_state["p_v99"] = f"{m_t}, {s_t}, {', '.join(k_t + w_t)}, {v_t}, {b_min}-{b_max} BPM"
            st.session_state["s_v99"] = generate_contents(title)
        if "p_v99" in st.session_state:
            # ONLY 2 BOXES AS COMMANDED. COPY SYMBOL IS AUTOMATICALLY IN st.code
            st.code(st.session_state["p_v99"], language="text")
            st.code(st.session_state["s_v99"], language="text")
